...
The CG method is $\sim 4$ times faster than the QR method: for the QR method we need n steps to compute the n $v_{i}$'s vector and the matrix $R$, while the CG method is an iterative method and we can chose after how many iterations to stop (CONSIDERAZIONI SUL FATTO CHE OLTRE UNA CERTA ACCURACY E QUINDI NUMERO DI ITERAZIONI LE COSE INIZIAVANO AD ANDARE MALE? FORSE È IL MOTIVO PER CUI IL RESIDUO DI CG È PIÙ ALTO DI QUELLO DI QR).
