...
In line 6 of Algorithm 4, we used \textit{numpy.linalg.inv} to compute the inverse of \textit{R}, which does a back substitution because \textit{R} is upper-triangular.
...
We are going to compare the two methods over the ML-cup in terms of time spent, memory usage and quality of the solution, and we will compare these results with other methods like SVD and normal equations .
...
Moreover, for the QR factorization, we are going to test how the computational cost of the factorization scales varying the dimension $m$ over the 3 matrices $m \times m$, $m \times n$ with $m > n$ and $m \times n$ with $m \gg n$.
