...
We can conclude that the space needed by CG is $O(n)$, while the space needed by QR is $O(n^{2})$, and so the CG method is more suitable for large matrices in terms of space.
...
(ABBOZZATA IN DUE MINUTI, SI PUò ANCHE TOGLIERE IN QUANTO NON RICHIESTA, MA SENZA CONCLUSIONE MI SEMBRAVA UN POCHINO TRISTE)
Both QR and CG methods did solve the high conditioned LS problem of ML-cup, but the QR method is two order of magnitude mode accurate and, as opposite as CG method, it doesn't require an initial point, a formula for $\beta$ and a number of iteration.
Anyway, the CG method was $\sim 4$ times faster than the QR one, and it is more suitable for large problem because the space it needs is $O(n)$ instead of $O(n^{2})$.
