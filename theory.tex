...
\State $\textit{Q} \gets \textit{eye(m)}$
\BState \emph{loop}:
\State $\textit{for j = 1:n}$
\State \quad$\textit{v} \gets \textit{householder-vector(A(j:end, j))}$
\State \quad$\textit{H} \gets \textit{eye(length(v)) - 2*v*v'}$
\State \quad$\textit{A(j:end, j:end)} \gets \textit{H * A(J.end, j:end)}$
\State \quad$\textit{Q(:, j:end)} \gets \textit{Q(:, j:end) * H}$
\BState \emph{end}:
\State $\textit{R = A}$
\EndFunction
\end{algorithmic}
...
\State \quad$\textit{Q(:, j:end)} \gets \textit{Q(:, j:end) - Q(:, j:end)*v*2*v'}$
\BState \emph{end}:
\State $\textit{R = A}$
\EndFunction
\end{algorithmic}
